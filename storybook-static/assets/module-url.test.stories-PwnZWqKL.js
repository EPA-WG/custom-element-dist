import{w as l,e as i}from"./index-CGuyH0k-.js";import"./custom-element-PxTx2W9y.js";import{f as c}from"./frame.canvas-BB4_DNn6.js";const d=(a,e)=>a.getAttribute(e);class u extends HTMLElement{static observedAttributes=["slice","src"];sliceInit(){let e=d(this,"src");try{const t=e.charAt(0)==="."?new URL(e,this.closest("[base]")?.getAttribute("base")||location.href).href:import.meta.resolve(e);this.setAttribute("value",this.value=t)}catch(t){this.setAttribute("error",t.message),this.setAttribute("value",e),console.error(t.message??t,e)}this.dispatchEvent(new Event("change"))}attributeChangedCallback(e,t,f){e==="src"&&this.sliceInit()}}window.customElements.define("module-url",u);function b(a){const{title:e,body:t}=a;return`
        <fieldset>
            <legend>${e}</legend>
            ${t}
        </fieldset>
  `}const p={title:"module-url",render:b,parameters:{test:{dangerouslyIgnoreUnhandledErrors:!0}}},n={args:{title:"1. relative to page path",body:`
    <custom-element>
        <template>
